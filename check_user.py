...
class Face_Recognizer:
    def __init__(self):
        ...
        self.detected_faces_dir = "static/detected_faces"
        if not os.path.exists(self.detected_faces_dir):
            os.makedirs(self.detected_faces_dir)

        # Dictionary to track the last time each face was saved
    ...
    def get_face_database(self):
        ...
        if os.path.exists("data/features_all.csv"):
            ...
            logging.warning(
                ...
                "and 'features_extraction_to_csv.py' before 'check_user.py'"
            )
            return 0
    ...
    # Save the detected face image and insert data into the database
    def found(self, name, img_rd, face_position):
        current_time = datetime.datetime.now()
        current_time_str = current_time.strftime("%H:%M:%S")
        current_date = current_time.strftime("%Y-%m-%d")

        # Check if the face was saved before
        if name in self.last_saved_time:
            last_saved_time = self.last_saved_time[name]
            time_diff = (current_time - last_saved_time).total_seconds() / 60  # Difference in minutes

            # If less than 5 minutes have passed, skip saving
            if time_diff < 5:
                print(f"{name} already saved within the last 5 minutes. Skipping.")
                return

        # Save the detected face as a JPEG file
        face_image_path = os.path.join(self.detected_faces_dir, f"{name}_{current_time_str.replace(':', '-')}.jpg")
        x, y, w, h = face_position
        face_img = img_rd[y:y + h, x:x + w]  # Extract the face region
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO found (name, time, date, image_path) VALUES (?, ?, ?, ?)",
            (name, current_time_str, current_date, face_image_path),
        )
        conn.commit()
        conn.close()

        # Update the last saved time for this face
        self.last_saved_time[name] = current_time
        print(f"{name} marked as present for {current_date} at {current_time_str}. Image saved at {face_image_path}")
                    ...
                    # 6.2.2 Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                # Save the detected face and insert data into the database
                                nam = self.face_name_known_list[similar_person_num]
                                face_position = (
                                    faces[k].left(),
                                    faces[k].top(),
                                    faces[k].right() - faces[k].left(),
                                    faces[k].bottom() - faces[k].top(),
                                )
                                self.found(nam, img_rd, face_position)
                            else:
