...
class Face_Register:
    def __init__(self):
        ...
        self.path_photos_from_camera = "data/known_faces/"
    ...
    def check_existing_faces_cnt(self):
        if os.listdir("data/known_faces/"):
            ...
            for person in person_list:
                person_order = ''.join(filter(str.isdigit, person))  # Extract numeric part from 'personx'
                if person_order.isdigit():  # Ensure it's a valid number
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            self.current_face_dir = self.path_photos_from_camera + \
                                    "person" + str(self.existing_faces_cnt) + "_" + \
                                    self.input_name_char
        else:
            self.current_face_dir = self.path_photos_from_camera + \
                                    "person" + str(self.existing_faces_cnt)
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "\"" + self.current_face_dir + "/face_" + str(
                        self.ss_cnt) + ".jpg\"" + " saved!"
                    self.face_ROI_image = cv2.cvtColor(self.face_ROI_image, cv2.COLOR_BGR2RGB)

                    cv2.imwrite(self.current_face_dir + "/face_" + str(self.ss_cnt) + ".jpg", self.face_ROI_image)
                    logging.info("%-40s %s/face_%s.jpg", "Save into：",
